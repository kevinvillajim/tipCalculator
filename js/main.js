...
//==============================Ejecutar funciones de calculo d en tips y total y mandar al inner============================= 

  const calculatedTip = calcularTip(bill, tip, people);
  const calculatedTotal = calcularTotal(bill, tip, people);

  tipAmountInput.textContent = `${calculatedTip.toFixed(2)}`;
  totalInput.textContent = `${calculatedTotal.toFixed(2)}`;

// Funcion para cambiar el color y activar .zero  

    esCero(people);

//=========================== Actualizar el estado de los botones en cada click (botones Porcentajes)=========================

    percentageButtons.forEach(button => {
      if (button.type === "button") {
          button.addEventListener("click", () => {
              claseActivo(percentageButtons, button);
              runProgram(button);
          });
      } else {
...
//====================== Comprobar la activación y desactivación en cada click (boton Reset)========================
//============================================No pude hacerle Funcionar  ='(   ='(   ='(   ='(   ='(   ='(   ='(   ='( 

  if (bill == 0 || people == 0){
    resetBtn.disabled = true;
    resetBtn.classList.add("buttonDes");
    
  } else {
    resetBtn.disabled = false
    resetBtn.classList.remove("buttonDes")
  }
...
resetBtn.addEventListener("click", () => {
  ...
  //======================== Desactivar boton reset y cambiar de estilo ==========================================
  //============================================No pude hacerle Funcionar  ='(   ='(   ='(   ='(   ='(   ='(   ='(
  if (bill === 0 && people === 0){
    resetBtn.disabled = true;
    resetBtn.classList.add("buttonDes")
  ...
  //============================Regresar todos los botones a la normalidad========================================
  percentageButtons.forEach((button)=> {
